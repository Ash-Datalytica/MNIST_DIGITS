    ...
    exclude = NULL, constant.weights = NULL, likelihood = FALSE) 
{
    ...
    for (i in 1:rep) {
        ...
        if (!is.null(result$output.vector)) {
          ...
          print ("Failed to return result under 'stepmax reached condition'")#Shovkun
        }
    }
    flush.console()
    if (!is.null(matrix)) {
    ...
    exclude, constant.weights, learningrate.bp) 
{
 ...
 ##Shovkun
 #Reove this code block to allow result to be returned if stepmax has been reached
#     if (reached.threshold > threshold) 
#         return(result = list(output.vector = NULL, weights = NULL))
 ##Shovkun
    ...
    return(list(generalized.weights = generalized.weights, weights = weights, 
        ...
        output.vector = output.vector, history=result$history))
}
...
function (model.list, hidden, startweights, rep, exclude, constant.weights) 
{
    ...
    if (!is.null(exclude)) {
        ...
        if (is.null(startweights) || length(startweights) < (length * 
            rep)) 
            #vector <- rnorm(length) #initial
            vector <- rnorm(length, mean=0, sd=1.22) #Shovkun, ref http://neuralnetworksanddeeplearning.com/chap3.html#weight_initialization
    ...
    exclude, learningrate.bp) 
{
    ...
    #Shovkun:
    history <- c(1, reached.threshold, sum(err.fct(result$net.result, response)))
    #history <- c(-1, reached.threshold, sum(err.fct(result$net.result, response)))
    while (step < stepmax && reached.threshold > threshold) {
        if (!is.character(lifesign) && step%%lifesign.step == 
            0) {
            error <- sum(err.fct(result$net.result, response))
            text <- paste("%", nchar.stepmax, "s", sep = "")
            cat(sprintf(eval(expression(text)), step), "\tmin thresh: ", 
                min.reached.threshold, ", error: ",error, 
                "\n", rep(" ", lifesign), 
                sep = "") #Shovkun
            flush.console()
            history<- rbind(history,c(step, min.reached.threshold, error)) #Shovkun
        }
        if (algorithm == "rprop+") 
    ...
    return(list(weights = weights, step = as.integer(step), reached.threshold = reached.threshold, 
        net.result = result$net.result, neuron.deriv = result$neuron.deriv, history=history))
}
compute.net <-
    ...
    learningrate, learningrate.factor, learningrate.limit, exclude) 
{
    ...
    negative <- temp < 0
    ##Shovkun Begin
#     cat("called plus, any positive=" , any(positive),"\n", sep = "")
#     flush.console()
    ##Shovkun End
    ...
    learningrate.limit, algorithm, exclude) 
{
    ...
    negative <- temp < 0
    ##Shovkun Begin
#     cat("called minus, any positive=" , any(positive),"\n", sep="")
#     flush.console()
    ##Shovkun End
    if (any(positive)) 
        learningrate[positive] <- pmin.int(learningrate[positive] * 
    ...
    linear.output, exclude) 
{
    ...
    if (!is.null(matrix)) {
        ...
        nn$history <-NULL #Shovkun
        #nn$lr <-list.result#Shovkun
        #nn$history2 <-list.result[[1]]$history#Shovkun
        for (i in 1:length(list.result)) {
            ...
            nn$history <- c(nn$history,list(list.result[[i]]$history))
        }
